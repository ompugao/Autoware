...
LocalMapPublisher::LocalMapPublisher (ros::NodeHandle &nh):
	rosnode (nh),
    left_sub(rosnode, "camera/left/image_raw", 3),
    right_sub(rosnode, "camera/right/image_raw", 3),
    imageSyncSub(MySyncPolicy(10), left_sub, right_sub),
	...
	lastGoodLocalizationTimestamp (-1)
{
	...
	SLAMSystem = new ORB_SLAM2::System (
        ...
        false,
        std::bind(&LocalMapPublisher::publishLocalMap, this, std::placeholders::_1)
        );
	...
	//imageSub = imageBuf->subscribe ("image_topic", 1, &LocalMapPublisher::imageCallback, this, th);

    imageSyncSub.registerCallback(boost::bind(&LocalMapPublisher::GrabStereo, this, _1, _2));

	// Result Publishers
	posePublisher = rosnode.advertise<geometry_msgs::PoseStamped> ("output/pose", 1);
	poseCovPublisher = rosnode.advertise<geometry_msgs::PoseWithCovarianceStamped> ("output/pose_covariance", 1);

	pointcloudPublisher = rosnode.advertise<sensor_msgs::PointCloud2> ("output/pointcloud", 1);
...
LocalMapPublisher::~LocalMapPublisher()
{
	SLAMSystem->Shutdown();
...
void LocalMapPublisher::updateQ() {
    float leftfx = SLAMSystem->fsSettings["Camera.fx"];
    float leftfy = SLAMSystem->fsSettings["Camera.fy"];
    float leftcx = SLAMSystem->fsSettings["Camera.cx"];
    float leftcy = SLAMSystem->fsSettings["Camera.cy"];
    float rightfx = SLAMSystem->fsSettings["CameraRight.fx"];
    float rightfy = SLAMSystem->fsSettings["CameraRight.fy"];
    float rightcx = SLAMSystem->fsSettings["CameraRight.cx"];
    float rightcy = SLAMSystem->fsSettings["CameraRight.cy"];
    float bf = SLAMSystem->fsSettings["Camera.bf"]; // baseline times fx
    double Tx = bf / rightfx;
    Q_(0,0) =  leftfy * Tx;
    Q_(0,3) = -leftfy * leftcx * Tx;
    Q_(1,1) =  leftfx * Tx;
    Q_(1,3) = -leftfx * leftcy * Tx;
    Q_(2,3) =  leftfx * leftfy * Tx;
    Q_(3,2) = -leftfy;
    Q_(3,3) =  leftfy * (leftcx - rightcx); // zero when disparities are pre-adjusted

}

void LocalMapPublisher::GrabStereo(const sensor_msgs::ImageConstPtr& msgLeft,const sensor_msgs::ImageConstPtr& msgRight) {

	ptime rT1, rT2;
	rT1 = microsec_clock::local_time();

    cv::Mat imgleft, imgright;
    if (!convertMsgToCV(msgLeft, imgleft)) {
        ROS_ERROR("cv_bridge failed to convert left image msg.");
        return;
    }
    if (!convertMsgToCV(msgRight, imgright)) {
        ROS_ERROR("cv_bridge failed to convert right image msg.");
        return;
    }
	const double imageTime = msgLeft->header.stamp.toSec();
	lastImageTimestamp = imageTime;
    msg_header_ = msgLeft->header;

	SLAMSystem->TrackStereo(imgleft, imgright, imageTime);
    ...
    for (int i = 0; i < cframe.N; i++) {
        //for each keypoint
        cv::Mat p = cframe.UnprojectStereo(i);
        if (!p.empty()) {
            cloud.push_back(pcl::PointXYZ(p.at<float>(2,0), -p.at<float>(0,0), -p.at<float>(1,0)));
        }
    }
    std::cout << ">> pointcloud has " << cloud.size() << " points at " << cframe.mTimeStamp << "(keypoints: " << cframe.N << std::endl;
    //cv::reprojectImageTo3D(disparity, point_cloud, Q_, false);
    sensor_msgs::PointCloud2 cloudmsg;
    pcl::toROSMsg(cloud, cloudmsg);
    cloudmsg.header = msgLeft->header;
    cloudmsg.header.stamp = ros::Time::now();
    pointcloudPublisher.publish(cloudmsg);
    */
...
bool LocalMapPublisher::convertMsgToCV(const sensor_msgs::ImageConstPtr& msg, cv::Mat& cvimg) {
	...
	if (enc::isBayer(msg->encoding)) {
		...
		cv::cvtColor(cv_ptr->image, cvimg, code);
	}
	else {
		cvimg = cv_ptr->image;
    }
...
void LocalMapPublisher::publishLocalMap(KeyFrame* currentKeyFrame) {
    vector<KeyFrame*> vpLocalKeyFrames = currentKeyFrame->GetVectorCovisibleKeyFrames();

    pcl::PointCloud<pcl::PointXYZ> cloud;

    cv::Mat cameraposeinv = currentKeyFrame->GetPose();//.rowRange(0,3).col(3);

    {
        const std::set<MapPoint*> vpMapPoints = currentKeyFrame->GetMapPoints();
        for(auto&& pMP : vpMapPoints)
        {
            //cv::Mat localpose = cameraposeinv * pMP->GetWorldPos();
            //cv::Mat pos = localpose.rowRange(0,3).col(3);
            cv::Mat globalpos = pMP->GetWorldPos();
            globalpos.resize(4);
            globalpos.at<float>(3) = 1;
            cv::Mat pos = cameraposeinv * globalpos;
            if (!pos.empty()) {
                cloud.push_back(pcl::PointXYZ(pos.at<float>(2,0), -pos.at<float>(0,0), -pos.at<float>(1,0)));
                //cloud.push_back(pcl::PointXYZ(pos.at<float>(0,0), pos.at<float>(1,0), pos.at<float>(2,0)));
            }
        }
    }
    for(vector<KeyFrame*>::iterator vit=vpLocalKeyFrames.begin(), vend=vpLocalKeyFrames.end(); vit!=vend; vit++)
    {
        ...
        const std::set<MapPoint*> vpMapPoints = pKF->GetMapPoints();
    ...
    sensor_msgs::PointCloud2 cloudmsg;
    pcl::toROSMsg(cloud, cloudmsg);
    cloudmsg.header = msg_header_;
    cloudmsg.header.stamp = ros::Time::now();
    pointcloudPublisher.publish(cloudmsg);
}

void
LocalMapPublisher::publishPose (const tf::Transform *pose)
{
...
tf::Transform
LocalMapPublisher::localizeByReference(const tf::Transform &tfOrb)
{
...
tf::Transform
LocalMapPublisher::localizeByReference (
...
tf::Transform
LocalMapPublisher::localizeByReference (const tf::Transform &tfOrb, ORB_SLAM2::KeyFrame *kf)
{
...
int main(int argc, char * argv[])
{
	...
	ros::init(argc, argv, "orb_stereo_proc", ros::init_options::AnonymousName);
	//ros::start();
	ros::NodeHandle nh("~");

	LocalMapPublisher l(nh);
	ros::spin();
